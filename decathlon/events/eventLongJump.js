  ...
  return {
    ...
    getStatusText(s) {
      ...
      if (s.phase === 'approach') {
        ...
        const hint = (s.appDice && s.appKept.length === 0)
          ? '　👆 まず1個クリックしてキープしよう！'
          : (s.appDice ? `　キープ合計: ${sum}` : '');
    ...
    getAvailableActions(s) {
      ...
      if (s.phase === 'approach') {
        ...
        return [
          ...
          { id: 'GO_JUMP', label: '🦘 ジャンプへ！', enabled: hasKept },
        ];
      }
    ...
    applyAction(s, action) {
      ...
      if (type === 'ROLL') {
        ...
        if (s.phase === 'approach') {
          ...
          s.appKeptSinceRoll = false;
          s.log.push(`[試技${s.attempt}][助走] 振り直し: [${s.appDice.join(', ')}] (残${s.appRollsLeft})`);
        } else if (s.phase === 'jump') {
