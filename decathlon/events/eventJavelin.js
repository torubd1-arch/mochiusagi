...
window.EventJavelin = (function () {
  ...
  return {
    ...
    getDescription() {
      return [
        '🎯 3回投げて、一番遠い記録が得点！',
        '【ルール】ダイス6個を振って、奇数（1・3・5）の目だけキープできるよ',
        '偶数（2・4・6）はキープできないよ。奇数が1個も出なかったらファウル（0点）',
    ...
    getDiceDisplay(s) {
      ...
      s.activeDice.forEach((v, i) => display.push({
        ...
        clickable: s.phase === 'rolling' && v % 2 === 1,
        dim: v % 2 === 0,
      }));
      return display;
    },
    ...
    getStatusText(s) {
      ...
      return `試技${s.attempt}/3 ／ キープ合計(奇数のみ): ${keptSum + selSum}点`;
    },
    ...
    getAvailableActions(s) {
      ...
      if (s.phase === 'rolling') {
        ...
        return [
          { id: 'ROLL', label: s.firstRoll ? '🎲 ロール（6個）' : `♻️ キープして振り直し（${unkeptCount}個）`, enabled: s.firstRoll || hasSelected },
    ...
    applyAction(s, action) {
      ...
      if (type === 'ROLL' && s.phase === 'rolling') {
        ...
        // ファウル判定：奇数が一個もなく、keptDiceも空の場合
        const hasOdd = s.activeDice.some(v => v % 2 === 1);
        if (!hasOdd && s.keptDice.length === 0) {
          s.foul = true;
          s.phase = 'foul';
          s.log.push(`[試技${s.attempt}] ファウル！奇数が1個も出なかった（全部偶数だよ）`);
        ...
        if (v % 2 === 1) {
          ...
          if (idx >= 0) {
            ...
            s.log.push(`[試技${s.attempt}] ${v}（奇数）をキープ選択！`);
          }
        } else {
          // 偶数：キープ不可
          s.log.push(`[試技${s.attempt}] ${v} は偶数だからキープできないよ！奇数（1・3・5）だけキープできるよ`);
        }
